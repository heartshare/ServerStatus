...
###
### Updated by Mark Krieger for uptime_used 2017-03-20
...
disk_info() {
  ...
  while read -r FSYS DSTOTAL DSUSED DSAVAIL DSPER FMOUNT; do if [ x"${FMOUNT}"x = x"${FS}"x ]; then G_HDDTOTAL=${DSTOTAL}; G_HDDFREE=${DSAVAIL}; G_HDDUSED=${DSUSED}; break; fi; done <<<"$(${DF_CMD} ${FS})"
}
...
GetMemInfo() {
  ...
  MEMMATH=$(( 100 - $MEMCACHE + $MEMFREE ))
  MEMORY=$(( $MEMMATH * 100 / $MEMTOTAL ))
  if [ $MEMORY -lt 75 ]; then
    MEMLEVEL="success"
...
<div class="bar bar-success" style="width: 100%"><small>Up - '${TIME}'</small></div>
</div>'
}
